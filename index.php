...
<body class="vh-100 text-center">
	<div class="container-fluid h-100">
    				...
    				<form class="text-left">
                            ...
                            <div class="col">
                                ...
                                <ul class="nav nav-tabs nav-justified mb-3" id="pills-tab" role="tablist">
                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link rounded-0 active" id="pills-url-tab" data-toggle="pill" href="#pills-url" role="tab" aria-controls="pills-url" aria-selected="true">URL</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link rounded-0" id="pills-file-tab" data-toggle="pill" href="#pills-file" role="tab" aria-controls="pills-file" aria-selected="false">File</a>
                                    </li>
                                </ul>
                                <div class="tab-content" id="pills-tabContent">
                                    <div class="tab-pane fade show active" id="pills-url" role="tabpanel" aria-labelledby="pills-url-tab">
                                        <input type="url" class="form-control" id="logo-image" name="logo-image" aria-describedby="urlHelp"/>
                                        <small id="urlHelp" class="form-text text-muted">Provide url for logo image.</small>
                                    </div>
                                    <div class="tab-pane fade" id="pills-file" role="tabpanel" aria-labelledby="pills-file-tab">
                                        <div class="custom-file">
                                            <input type="file" class="custom-file-input" id="logo-file" accept="image/png, image/jpeg, image/svg+xml" aria-describedby="fileHelp"/>
                                            <label class="custom-file-label" for="logo-file">Choose file</label>
                                        </div>
                                        <small id="fileHelp" class="form-text text-muted">Choose file for logo image.</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-row mb-3">
                            <div class="col">
                                ...
                                <input type="text" class="form-control" id="store-address" name="store-address" aria-describedby="storeAddress"/>
                                <small id="storeAddress" class="form-text text-muted">Enter Store Address.</small>
                            </div>
                        </div>
    					<div class="form-row mt-5">
    						<div class="col-xl mb-3">
    							<button type="button" class="btn btn-primary btn-block" data-toggle="tooltip" data-placement="top" id="download-image" title="Download Image"><i class="fa fa-image"></i></button>
    						</div>
                            <div class="col-xl mb-3">
                                <button type="button" class="btn btn-success btn-block" data-toggle="tooltip" data-placement="top" id="download" title="Download Package"><i class="fa fa-download"></i></button>
                            </div>
    						<div class="col-xl mb-3">
    							<button type="button" class="btn btn-info btn-block" data-toggle="tooltip" data-placement="top" id="preview" title="Preview New Tab"><i class="fa fa-eye"></i></button>
    						</div>
    					</div>
    				</form>
                </div>
	 ...
	 flex-shrink-1 flex-basis-0 overflow-auto style-scroll">
...
(function($) {
	...
	$(document).ready(($) => {
        ...
        const loadPreview = () => {
        	...
        	mailPreview.attr('src', encodeURI(`${window.location.href}templates/smb/${paramPath}`));
        ...
        const readFile = async (file, fileName) => {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {fileName: fileName, logo: base64Logo},
            })
        ...
        download.on('click', (event) => {
        	$.ajax({
        		...
        		type: 'POST',
        		data: {mail: mailPreview.attr('src')},
        	})
        	.done(function(response) {
        		if(response.status){
        ...
        downloadImage.on('click', (event) => {
            ...
            html2canvas(exportElement,{useCORS:true, width:600,})
